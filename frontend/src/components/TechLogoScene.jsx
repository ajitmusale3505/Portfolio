...
function RaisedBadge({ children, label, color, accent, position, rotation = [0, 0, 0] }) {
  ...
  return (
    <Float speed={1.65} rotationIntensity={0.1} floatIntensity={0.26}>
      ...
      >
        <RoundedBox args={[1.24, 1.24, 0.24]} radius={0.18} smoothness={10}>
        ...
        <RoundedBox args={[1.02, 1.02, 0.08]} radius={0.15} smoothness={8} position={[0, 0, 0.16]}>
          <meshPhysicalMaterial
            color={color}
            emissive={accent}
    ...
    <group scale={0.88}>
      {[0, Math.PI / 3, -Math.PI / 3].map((rotation) => (
        <mesh key={rotation} rotation={[Math.PI / 2, 0, rotation]}>
          <torusGeometry args={[0.33, 0.025, 18, 96]} />
  ...
  return (
    <group scale={0.82}>
      <mesh position={[0, -0.08, 0]} rotation={[0, 0, 0]}>
        <cylinderGeometry args={[0.25, 0.31, 0.32, 64]} />
  ...
  return (
    <group scale={0.9} rotation={[0, 0, -0.22]}>
      ...
      <mesh position={[0.08, 0.03, 0.09]} rotation={[0, 0, -0.72]}>
        <torusGeometry args={[0.27, 0.018, 10, 64, Math.PI * 1.2]} />
    ...
    <group scale={0.74}>
      {[
        ...
        <mesh key={index} position={[x, y, 0]}>
          <cylinderGeometry args={[0.38, 0.38, 0.18, 64]} />
...
function LogoRig() {
  ...
  return (
    <group ref={rig}>
      ...
      <ContactShadows position={[0.18, -1.52, -0.18]} opacity={0.62} blur={2.2} scale={8.2} far={2.8} color="#030712" />
    </group>
  )
}
...
export default function TechLogoScene() {
  return (
    <Canvas camera={{ position: [0, 0.1, 6.4], fov: 38 }} dpr={[1, 1.8]} shadows>
      <ambientLight intensity={0.42} />
      <spotLight position={[-3.4, 4.4, 5.8]} angle={0.46} penumbra={0.7} intensity={6.5} color="#ffffff" castShadow />
